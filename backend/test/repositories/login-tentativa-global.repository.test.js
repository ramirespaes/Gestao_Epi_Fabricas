...
} = require('../../src/repositories/login-tentativa-global.repository');

/**
 * Contrato do repositório de tentativas de login do Painel Privado da
 * plataforma (migration 036 — correção final do Pacote 2, item 1).
 * Espelha login-tentativa.repository.test.js; diferença estrutural: só
 * `identidadeId` (vínculo simples), sem `empresaId`.
 */
...
describe('registrarTentativa', () => {
  test('grava tentativa bem-sucedida, com identidade, sem motivo', async () => {
    const executor = executorFalso([{ id: ID_BIGINT }]);

    const id = await registrarTentativa(executor, { chaveCooldown: CHAVE, identidadeId: IDENTIDADE_ID, sucesso: true });

    assert.equal(id, ID_BIGINT, 'o identificador BIGINT não pode ser convertido para Number');
    const { texto, valores } = executor.chamadas[0];
    assert.match(texto, /insert\s+into\s+login_tentativas_globais/i);
    assert.match(texto, /clock_timestamp\(\)/i);
    assert.deepEqual(valores, [CHAVE, IDENTIDADE_ID, true, null, null, null]);
    assert.equal(texto.includes(CHAVE), false, 'a chave não pode ser concatenada no SQL');
  });

  test('grava tentativa malsucedida sem identidade identificada (IDENTIDADE_INEXISTENTE)', async () => {
    const executor = executorFalso([{ id: ID_BIGINT }]);

    await registrarTentativa(executor, { chaveCooldown: CHAVE, sucesso: false, motivo: 'IDENTIDADE_INEXISTENTE' });

    const { valores } = executor.chamadas[0];
    assert.deepEqual(valores, [CHAVE, null, false, 'IDENTIDADE_INEXISTENTE', null, null]);
  });
  ...
  test('grava tentativa malsucedida totalmente identificada (SENHA_INVALIDA)', async () => {
    ...
    await registrarTentativa(executor, {
      chaveCooldown: CHAVE, identidadeId: IDENTIDADE_ID, sucesso: false,
      motivo: 'SENHA_INVALIDA', ip: '203.0.113.10', dispositivo: 'navegador de teste',
    });

    const { valores } = executor.chamadas[0];
    assert.deepEqual(valores, [CHAVE, IDENTIDADE_ID, false, 'SENHA_INVALIDA', '203.0.113.10', 'navegador de teste']);
  });
  ...
  test('recusa tentativa bem-sucedida com motivo', async () => {
    ...
    await assert.rejects(() => registrarTentativa(executor, {
      chaveCooldown: CHAVE, identidadeId: IDENTIDADE_ID, sucesso: true, motivo: 'SENHA_INVALIDA',
    }), /motivo/i);
    assert.equal(executor.chamadas.length, 0);
  });

  test('recusa tentativa bem-sucedida sem identidade identificada', async () => {
    const executor = executorFalso([{ id: ID_BIGINT }]);
  ...
  test('recusa identificador de identidade inválido', async () => {
    const executor = executorFalso([{ id: ID_BIGINT }]);

    await assert.rejects(() => registrarTentativa(executor, { chaveCooldown: CHAVE, identidadeId: 0, sucesso: false, motivo: 'SENHA_INVALIDA' }), /identidade/i);
    await assert.rejects(() => registrarTentativa(executor, { chaveCooldown: CHAVE, identidadeId: -1, sucesso: false, motivo: 'SENHA_INVALIDA' }), /identidade/i);
    await assert.rejects(() => registrarTentativa(executor, { chaveCooldown: CHAVE, identidadeId: '9', sucesso: false, motivo: 'SENHA_INVALIDA' }), /identidade/i);
    assert.equal(executor.chamadas.length, 0);
  });
});
...
describe('registrarAtivacaoCooldown', () => {
  ...
  test('grava a ativação vinculada à identidade quando informada', async () => {
    const executor = executorFalso([{ id: ID_BIGINT }]);
    const cooldownAte = new Date(Date.now() + 900_000);

    await registrarAtivacaoCooldown(executor, { chaveCooldown: CHAVE, cooldownAte, identidadeId: IDENTIDADE_ID });

    const { valores } = executor.chamadas[0];
    assert.deepEqual(valores, [CHAVE, IDENTIDADE_ID, MOTIVO_COOLDOWN_ATIVADO, cooldownAte, null, null]);
  });
