...
const identidadeBase = Object.freeze({
  id: IDENTIDADE_ID,
  email: EMAIL,
  senhaHash: '$argon2id$v=19$m=65536,t=3,p=1$c2ludGV0aWNv$aGFzaHNpbnRldGljbw',
  ativo: true,
});
const identidadeInativa = Object.freeze({ ...identidadeBase, ativo: false });
...
describe('autenticar — login válido', () => {
  test('registra a tentativa como sucesso, cria a sessão e devolve token/expiração coerentes', async (t) => {
    t.mock.method(identidadeRepo, 'buscarCredencialPorEmail', async () => identidadeBase);
    ...
    assert.equal(registrar.mock.calls[0].arguments[1].identidadeId, IDENTIDADE_ID);

    assert.equal(criarSessao.mock.calls.length, 1);
    const argsCriar = criarSessao.mock.calls[0].arguments[1];
    assert.equal(argsCriar.identidadeId, IDENTIDADE_ID);
  ...
  test('e-mail passa por normalizarEmail antes de consultar o repositório', async (t) => {
    const buscar = t.mock.method(identidadeRepo, 'buscarCredencialPorEmail', async () => identidadeBase);
    t.mock.method(password, 'verificarSenha', async () => true);
    t.mock.method(loginTentativaGlobalRepo, 'buscarCooldownVigente', async () => null);
    t.mock.method(loginTentativaGlobalRepo, 'registrarTentativa', async () => '1');
    t.mock.method(sessaoRepo, 'criar', async () => SESSAO_ID);

    await autenticar(criarPoolFalso(criarClienteFalso()), { email: '  Pessoa@Exemplo-Cliente.com.br  ', senha: SENHA });
...
describe('autenticar — falhas de credencial, todas com resposta pública genérica', () => {
  const casos = [
    {
      ...
      setup: (t) => {
        ...
        return { senhaReal, senhaFicticia, esperaSenhaReal: true, esperaSenhaFicticia: false, identidadeId: IDENTIDADE_ID };
      },
    },
    {
      nome: 'identidade inativa, mesmo com senha correta',
      motivoEsperado: 'IDENTIDADE_INATIVA',
      setup: (t) => {
        t.mock.method(identidadeRepo, 'buscarCredencialPorEmail', async () => identidadeInativa);
    ...
    },
    {
      ...
      motivoEsperado: 'IDENTIDADE_INEXISTENTE',
      setup: (t) => {
        const buscar = t.mock.method(identidadeRepo, 'buscarCredencialPorEmail', async () => null);
        const senhaReal = t.mock.method(password, 'verificarSenha', async () => true);
        const senhaFicticia = t.mock.method(password, 'verificarSenhaContraFicticio', async () => false);
        return {
          senhaReal, senhaFicticia, esperaSenhaReal: false, esperaSenhaFicticia: true, identidadeId: null, buscar,
        };
      },
    },
  ];
  ...
  for (const caso of casos) {
    test(`${caso.nome}: 401 genérico, motivo ${caso.motivoEsperado} persistido, sem sessão`, async (t) => {
      ...
      t.mock.method(loginTentativaGlobalRepo, 'buscarCooldownVigente', async () => null);
      const registrar = t.mock.method(loginTentativaGlobalRepo, 'registrarTentativa', async () => '1');
      const contarFalhas = t.mock.method(loginTentativaGlobalRepo, 'contarFalhasRecentes', async () => 0);
      const criarSessao = t.mock.method(sessaoRepo, 'criar', async () => SESSAO_ID);
      ...
      assert.equal(args.identidadeId, contexto.identidadeId);
...
describe('autenticar — cooldown vigente', () => {
  test('não verifica senha nem registra nova tentativa; responde 429 com Retry-After', async (t) => {
    ...
    t.mock.method(loginTentativaGlobalRepo, 'buscarCooldownVigente', async () => ({ ativoAte }));
    const buscarAdministrador = t.mock.method(identidadeRepo, 'buscarCredencialPorEmail', async () => identidadeBase);
    const senhaReal = t.mock.method(password, 'verificarSenha', async () => true);
    const senhaFicticia = t.mock.method(password, 'verificarSenhaContraFicticio', async () => false);
    const registrar = t.mock.method(loginTentativaGlobalRepo, 'registrarTentativa', async () => '1');
...
describe('autenticar — ativação de cooldown', () => {
  test('nível 1: ativa com a duração do primeiro nível ao cruzar o limiar de 15 min', async (t) => {
    ...
    t.mock.method(loginTentativaGlobalRepo, 'buscarCooldownVigente', async () => null);
    t.mock.method(loginTentativaGlobalRepo, 'registrarTentativa', async () => '1');
    const ativar = t.mock.method(loginTentativaGlobalRepo, 'registrarAtivacaoCooldown', async () => '2');

    let chamada = 0;
    t.mock.method(loginTentativaGlobalRepo, 'contarFalhasRecentes', async () => {
      chamada += 1;
  ...
  test('sem cruzar nenhum limiar, não ativa cooldown', async (t) => {
    ...
    t.mock.method(loginTentativaGlobalRepo, 'buscarCooldownVigente', async () => null);
    t.mock.method(loginTentativaGlobalRepo, 'registrarTentativa', async () => '1');
    t.mock.method(loginTentativaGlobalRepo, 'contarFalhasRecentes', async () => 1);
    const ativar = t.mock.method(loginTentativaGlobalRepo, 'registrarAtivacaoCooldown', async () => '2');
...
describe('autenticar — ROLLBACK em erros inesperados', () => {
  test('erro em qualquer chamada de repositório aciona ROLLBACK e propaga', async (t) => {
    ...
    t.mock.method(loginTentativaGlobalRepo, 'registrarTentativa', async () => { throw erroInesperado; });
    const criarSessao = t.mock.method(sessaoRepo, 'criar', async () => SESSAO_ID);
