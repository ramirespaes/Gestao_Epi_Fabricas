 ...
 * middleware/autenticacao.test.js: sessao-plataforma.repository é sempre
...
const CONTEXTO_SESSAO = Object.freeze({
  ...
  administrador: { id: 9, email: 'admin@safework.com.br' },
});

function montarApp(middleware) {
  return criarAppTeste((app) => {
    app.get('/painel', middleware, (req, res) => {
      res.json({ sessaoPlataforma: req.sessaoPlataforma, administradorPlataforma: req.administradorPlataforma });
    });
  });
}

function cookieValido(token = TOKEN) {
  return `${authConfig.sessao.cookieNomeAdmin}=${token}`;
}

describe('criarExigirSessaoPlataforma', () => {
  ...
  test('cookie com formato inválido: 401, hashTokenSessao/repositório nunca chamados', async (t) => {
    ...
    const resposta = await request(montarApp(middleware))
      .get('/painel')
      .set('Cookie', `${authConfig.sessao.cookieNomeAdmin}=nao-eh-um-token-valido`);
  ...
  test('buscarValidaPorHash chamado com pool injetado, hash correto e inatividade configurada', async (t) => {
    const buscar = t.mock.method(sessaoPlataformaRepo, 'buscarValidaPorHash', async () => CONTEXTO_SESSAO);
    t.mock.method(sessaoPlataformaRepo, 'registrarUso', async () => true);
    const poolFalso = { marcador: 'pool-de-teste' };
    const middleware = criarExigirSessaoPlataforma({ pool: poolFalso });
  ...
  test('buscarValidaPorHash devolve null: 401, registrarUso nunca chamado', async (t) => {
    t.mock.method(sessaoPlataformaRepo, 'buscarValidaPorHash', async () => null);
    const registrar = t.mock.method(sessaoPlataformaRepo, 'registrarUso', async () => true);
    const middleware = criarExigirSessaoPlataforma({ pool: {} });
  ...
  test('registrarUso devolve false: 401, a rota protegida não é alcançada', async (t) => {
    t.mock.method(sessaoPlataformaRepo, 'buscarValidaPorHash', async () => CONTEXTO_SESSAO);
    t.mock.method(sessaoPlataformaRepo, 'registrarUso', async () => false);
    const middleware = criarExigirSessaoPlataforma({ pool: {} });

    const resposta = await request(montarApp(middleware)).get('/painel').set('Cookie', cookieValido());

    assert.equal(resposta.status, 401);
    assert.equal('administradorPlataforma' in resposta.body, false);
  });

  test('sessão válida: next() chamado, req.sessaoPlataforma/administradorPlataforma populados, nunca req.usuario/req.empresa', async (t) => {
    t.mock.method(sessaoPlataformaRepo, 'buscarValidaPorHash', async () => CONTEXTO_SESSAO);
    t.mock.method(sessaoPlataformaRepo, 'registrarUso', async () => true);
    const middleware = criarExigirSessaoPlataforma({ pool: {} });
    const app = criarAppTeste((a) => {
      a.get('/painel', middleware, (req, res) => {
        res.json({
          sessaoPlataforma: req.sessaoPlataforma,
          administradorPlataforma: req.administradorPlataforma,
    ...
    assert.equal(resposta.body.sessaoPlataforma.id, CONTEXTO_SESSAO.sessao.id);
    assert.equal(resposta.body.administradorPlataforma.id, CONTEXTO_SESSAO.administrador.id);
  ...
  test('erro inesperado em buscarValidaPorHash propaga (500), não vira 401', async (t) => {
    t.mock.method(sessaoPlataformaRepo, 'buscarValidaPorHash', async () => { throw new Error('conexão perdida com o banco'); });
    const middleware = criarExigirSessaoPlataforma({ pool: {} });
  ...
  test('nenhuma resposta 401/500 expõe o token ou o hash, nem no corpo nem no log', async (t) => {
    ...
    t.mock.method(sessaoPlataformaRepo, 'buscarValidaPorHash', async () => { throw new Error(`falha com token ${TOKEN}`); });
    const middleware = criarExigirSessaoPlataforma({ pool: {} });
